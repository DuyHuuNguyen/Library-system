...
/**
 * build a button in easiest way, just set the attribute that you need -.-
 * example:
 * CustomButton myButton = new CustomButtonBuilder("Login")
 *     .setFont(new Font("Arial", Font.BOLD, 18))
 *     .setTextColor(Color.WHITE)
 *     .setBackgroundColor(new Color(66, 135, 245))
 *     .setHoverColor(new Color(66, 100, 200))
 *     .setBorderColor(Color.BLUE)
 *     .setGradientColors(Color.CYAN, Color.BLUE)   rarely used!
 *     .setBorderThickness(4)
 *     .setBorderRadius(20)
 *     .setDrawBorder(true)
 *     .setDarkerWhenPress(true)
 *     .setTextPosition(SwingConstants.CENTER)
 *     .setSize(new Dimension(150, 50))
 *     .icon("src/main/resources/icons/login.png", 10)
 *     .build();
 */
public class CustomButtonBuilder {
    ...
    public CustomButton build() {
        ...
        if (iconPath != null) {
            ...
            Image resized = image.getScaledInstance(
                    size.height - iconGap, size.height - iconGap, Image.SCALE_SMOOTH);
    ...
    public static void main(String[] args) {
        CustomButton myButton = new CustomButtonBuilder()
                .text("Đăng nhập")
