...
import com.g15.library_system.view.overrideComponent.dateChoosers.layout.CellLayout;
import com.g15.library_system.view.overrideComponent.dateChoosers.layout.HeaderLayout;
...
import com.g15.library_system.view.overrideComponent.dateChoosers.render.DateChooserRender;
import com.g15.library_system.view.overrideComponent.dateChoosers.render.DefaultDateChooserRender;
import java.awt.*;
...
public class DateChooser extends JPanel {
  ...
  public DateBetween getSelectedDateBetween() throws DateChooserException {
    ...
    DateBetween dateBetween =
        new DateBetween(selectedDateBetween[0].toDate(), selectedDateBetween[1].toDate());
    dateBetween.fixDate();
    return dateBetween;
  }
  ...
  private void createHeader() {
    ...
    panelHeader.putClientProperty(
        FlatClientProperties.STYLE,
        "" + "background:if($DateChooser.background,$DateChooser.background,$Panel.background)");
    JButton cmdBack = new JButton(new FlatSVGIcon("icons/arrowForward.svg"));
    JButton cmdNext = new JButton(new FlatSVGIcon("icons/arrowPrevious.svg"));
    ...
    panelTitle.putClientProperty(
        FlatClientProperties.STYLE,
        "" + "background:if($DateChooser.background,$DateChooser.background,$Panel.background)");
    spMonth.putClientProperty(
        FlatClientProperties.STYLE,
        ""
            + "background:if($DateChooser.background,$DateChooser.background,$Panel.background);"
            + "buttonBackground:if($DateChooser.background,$DateChooser.background,$Panel.background)");
    spYear.putClientProperty(
        FlatClientProperties.STYLE,
        ""
            + "background:if($DateChooser.background,$DateChooser.background,$Panel.background);"
            + "buttonBackground:if($DateChooser.background,$DateChooser.background,$Panel.background)");
    cmdBack.putClientProperty(
        FlatClientProperties.STYLE,
        "" + "background:if($DateChooser.background,$DateChooser.background,$Panel.background)");
    cmdNext.putClientProperty(
        FlatClientProperties.STYLE,
        "" + "background:if($DateChooser.background,$DateChooser.background,$Panel.background)");
  ...
  private void createLabelCurrentDate() {
    ...
    labelCurrentDate.addActionListener(
        e -> {
          if (dateSelectionMode == DateSelectionMode.SINGLE_DATE_SELECTED) {
            toDay();
          } else {
  ...
  private void showDate() {
    ...
    for (int i = 1; i <= 7 * 6; i++) {
      ...
      if (calendar.get(Calendar.MONTH) != month) {
        cmd.putClientProperty(FlatClientProperties.STYLE, "" + "foreground:$Button.disabledText");
      }
      panelDate.add(cmd);
    ...
    public Dimension preferredLayoutSize(Container parent) {
      synchronized (parent.getTreeLock()) {
        ...
        int height =
            insets.top
                + insets.bottom
                + (panelHeader.getPreferredSize().height
                    + UIScale.scale(2)
                    + panelTitle.getPreferredSize().height
                    + panelDate.getPreferredSize().height);
